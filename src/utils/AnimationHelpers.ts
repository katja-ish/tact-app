...
export const withSpring = (props: WithSpringParams) => {
  const { value, velocity, state, snapPoints, offset, config: springConfig, onSnap } = {
    offset: new Value(0),
    ...props,
  };
  const clock = new Clock();
  ...
  const snap = onSnap ? [cond(clockRunning(clock), call([springState.position], onSnap))] : [];
  return block([
...
export const withOffset = ({
  ...
  return block([
    cond(not(defined(safeOffset)), set(safeOffset, offset === undefined ? 0 : offset)),
