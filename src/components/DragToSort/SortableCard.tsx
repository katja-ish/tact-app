...
import Card, { CardProps, CARD_HEIGHT as INNER_CARD_HEIGHT } from './Card';
...
const isMoving = (position: Animated.Node<number>) => {
  ...
  return cond(
    lessThan(abs(delta), 1),
    [set(noMovementFrames, add(noMovementFrames, 1)), not(greaterThan(noMovementFrames, 20))],
...
export default ({ item, index, offsets }: SortableCardProps) => {
  ...
  const zIndex = cond(eq(state, State.ACTIVE), 200, cond(isMoving(translateY), 100, 1));
